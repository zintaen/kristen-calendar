...
- NIT (not fixed) - TASK-020's migration is `0019_entitlements.sql`; because TASK-017 and TASK-018 both create `0016_*`, the 0016-0019 sequence needs consistent renumbering when the whole P3 branch merges (see the number-collision note in the TASK-017/018 audits). `genie_usage_monthly` is declared in migration 0019 but not listed in TASK-020's frontmatter `new_files` (only the overall .sql file is listed) - cosmetic. Post-fix: 10/10.
