...
related_frs: [TASK-LUNAR-001, TASK-LUNAR-011]
depends_on: [TASK-LUNAR-001]
blocks: [TASK-LUNAR-003, TASK-LUNAR-007, TASK-LUNAR-011, TASK-LUNAR-013]
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-A03, TASK-A04)"
...
disallowed_tools:
  ...
  - "using Chinese zodiac animals (Rabbit/Ox) instead of VN (Cat/Buffalo) (violates DEC-LUNAR-021 / TASK-A03)"
effort_hours: 10.5
sub_tasks:
  ...
  - "0.5h: tietKhiStartDiaChi(jdn, tz?) - the Earthly Branch (0..11) of the tiet start day; TASK-011 uses it to compute Truc (CONTRACT.md)"
...
risk_if_skipped: "Without can-chi and tiet khi, the TASK-LUNAR-007 month calendar is missing information in every day cell, TASK-LUNAR-011 day quality has no input (Hoang dao/Truc/28 sao are all computed from the day can-chi), TASK-LUNAR-003 validation cannot assert the fixture's can-chi year, and the TASK-LUNAR-013 widget cannot show can-chi. A wrong zodiac (using Rabbit instead of Cat) is a VN-specific error that the persona Chi Linh notices immediately (Tet 2023 must be the Cat)."
---

## §1 - Description (BCP-14 normative)

The module MUST compute the day/month/year can-chi, the animal per the Vietnamese zodiac, and the 24 tiet khi (12 of which are Principal Terms), all timezone-independent and derived from the JDN of TASK-LUNAR-001. Contract:

1. MUST compute the day can-chi from the JDN: `canIndex = (jdn + 9) % 10` and `chiIndex = (jdn + 1) % 12`, returning the name assembled from the table of 10 Can and 12 Chi (TASK-A03, DEC-LUNAR-020).
...
3. MUST compute the year can-chi from `lunarYear`: `can = (lunarYear + 6) % 10`, `chi = (lunarYear + 8) % 12` (TASK-A03, DEC-LUNAR-023).
4. MUST compute the month can-chi from `lunarMonth` and the year's Can: `monthCan = (yearCanIndex * 2 + lunarMonth + 1) % 10`, with the month Chi fixed starting at Dan for the first month (TASK-A03, DEC-LUNAR-022).
5. MUST use the Vietnamese zodiac: the animal at chi Mao is "Meo" (the Cat, not the Rabbit), at chi Suu it is "Trau" (the Buffalo, not the Ox); provide a full 12-animal VN table (TASK-A03, DEC-LUNAR-021).
6. MUST provide `zodiacOf(chiIndex)` returning the VN animal corresponding to chi index 0..11 (CONTRACT.md); the caller computes `chiIndex = (lunarYear + 8) % 12` to convert from the lunar year (TASK-A03).
...
7. MUST compute tiet khi from `getSunLongitude` at 15-degree resolution: `tietKhiIndex = INT(SunLongitude(jdn - 0.5 - tz / 24) / PI * 12)` returning 0-23 (TASK-A04, DEC-LUNAR-024).
8. MUST provide a table of the 24 tiet khi in Vietnamese with diacritics in the correct order, marking the 12 that are Principal Terms (TASK-A04).
9. MUST ensure the Dong chi (Winter Solstice) is a Principal Term and the anchor that determines lunar month 11, consistent with `getLunarMonth11` of TASK-LUNAR-001 (TASK-A04, PRD 6.1 rule 3).
...
11. MUST provide `tietKhiAt(jdn, tz?)` returning a `TietKhi` with `index` 0..23, `name`, and `isTrungKhi` (TASK-A04, CONTRACT.md).
11a. MUST provide `tietKhiStartDiaChi(jdn, tz?): number` returning the Earthly Branch (0..11) of the day the current tiet khi containing `jdn` starts; TASK-LUNAR-011 uses it to compute Truc via the formula `(diaChiNgay - tietKhiStartDiaChi + 12) % 12` (CONTRACT.md, DEC-LUNAR-024).
...
13. MUST export the `CanChi`, `TietKhi`, `Zodiac` types through the `index.ts` barrel so TASK-LUNAR-007 and TASK-LUNAR-011 can reuse them (DEC-LUNAR-020).
...
**Why does the month can-chi depend on the year's Can?** The month Chi is fixed (the first month is Dan, the second is Mao...) but the month Can rotates with the year Can. The formula `(yearCanIndex * 2 + lunarMonth + 1) % 10` gives the correct month Can, consistent with the traditional method. This is the input for Hoang dao/Hac dao in TASK-LUNAR-011 (DEC-LUNAR-022).

**Why does tiet khi use getSunLongitude at 15-degree resolution?** A full 360-degree zodiac divided into 24 tiet khi gives 15 degrees each. TASK-LUNAR-001 already has `getSunLongitude` at 30-degree resolution (dividing by 6, returning 0-11) for Principal Terms; here it divides by 12 to yield the 24 tiet khi (`/ PI * 12`). Reusing the exact same SunLongitude function ensures tiet khi is consistent with how the core determines the leap month (DEC-LUNAR-024).
 ...
 * TASK-011 dung ham nay de tinh Truc: trucIndex = (diaChiNgay - tietKhiStartDiaChi + 12) % 12.
...
11. The start day of lunar month 11 (`getLunarMonth11`) from TASK-LUNAR-001 contains the Dong chi: `tietKhiAt` of a day in lunar month 11 near the Dong chi returns the Dong chi Principal Term (consistent with the core, TASK-A04).
...
describe("tiet khi (TASK-A04, DEC-LUNAR-024)", () => {
...
The API in §3 is a full skeleton. The detail worth pinning: tiet khi and Principal Terms use the SAME `SunLongitude` function from TASK-LUNAR-001 but at different resolutions - Principal Terms divide by 6 (30-degree resolution, in the core), tiet khi divide by 12 (15-degree resolution, here). Both must take `jdn - 0.5 - tz/24` as the input so the two places agree about the Dong chi. AC #11 is the consistency constraint between this tiet khi table and the core's `getLunarMonth11`.

---

## §7 - Dependencies

Upstream: TASK-LUNAR-001 - uses `jdFromDate`, `jdToDate`, `SunLongitude`, and `VN_TIMEZONE`. The names match the depends_on frontmatter.

Downstream: TASK-LUNAR-003 asserts the can-chi year of the Tet fixtures; TASK-LUNAR-007 shows can-chi and tiet khi on every calendar cell; TASK-LUNAR-011 uses the day can-chi as the input for Hoang dao/Hac dao/Truc/28 sao; TASK-LUNAR-013 shows can-chi on the widget. The names match the blocks frontmatter.
...
Deferred:
- The deeper than sat (deity/malefic) systems (Truc details, detailed lucky/unlucky stars) live in TASK-LUNAR-011, not here; this task only handles can-chi/zodiac/tiet khi as the foundation.
- Whether to add Han-Viet names for the tiet khi with the Chinese characters - deferred to the TASK-LUNAR-008 content layer if richer display is needed.
...
| Month can-chi does not rotate with the year Can | month Can wrong | TASK-011 input wrong | the DEC-LUNAR-022 formula |
...
- The month can-chi has a fixed Chi (the first month is Dan) but the Can rotates with the year Can; the formula is `(yearCan * 2 + lunarMonth + 1) % 10`. This is a required input for the Hoang dao/Hac dao table by day Earthly Branch x month in TASK-LUNAR-011.
