...
related_frs: [TASK-LUNAR-016]
depends_on: [TASK-LUNAR-004, TASK-LUNAR-016]
blocks: []
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-B08)"
...
1. MUST send ZNS only to phone numbers the user voluntarily provided via the Zalo Mini App (TASK-LUNAR-016) and that have a transactional relationship with the OA; MUST NOT send to phone numbers collected from any other source or without consent (DEC-LUNAR-170, NFR-Privacy/PDPL).
...
**Why ZNS instead of a regular push notification?** The Zalo Mini App has no native push API (DEC-LUNAR-162 in TASK-016). ZNS via the OA is the only way for "Genie Am Lich" to send proactive messages to Zalo users. Beyond that, ZNS has a higher delivery rate than web push because it goes through Zalo's official messaging system of roughly 2.1 billion messages/day (PRD Key Findings 3).
  ...
  for (const reminder of reminders) {
    ...
    for (const { lunarYear, lunarMonth } of candidates) {
      for (const leadDays of reminder.leadTimes) {
        // convertLunar2Solar trả về tuple SolarDate = [dd, mm, yy] (TASK-LUNAR-001);
...
Upstream: **TASK-LUNAR-004** (the `Reminder` data model; the fields `lunarDay`, `lunarMonth`, `isLeapMonth`, `channels`, `enabled` are used directly in `SchedulerReminder`). **TASK-LUNAR-016** (the Zalo Mini App client provides the `User.phone` token and the list of Reminders with `channels: ["ZNS"]`; without TASK-016 there is no data source).

Downstream: No task depends on TASK-017.

Cross-cutting: `@cyberskill/amlich-core` is used in the cron (server-side) to compute the solar date from the lunar date; consistent with the client. `zns_send_log` is the data source for ZNS cost analytics in TASK-LUNAR-020 (freemium billing).
