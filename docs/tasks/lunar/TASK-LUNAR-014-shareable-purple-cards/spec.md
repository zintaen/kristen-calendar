...
related_frs: [TASK-LUNAR-009]
depends_on: [TASK-LUNAR-007, TASK-LUNAR-009]
blocks: []
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-F03)"
...
source_decisions:
  ...
  - DEC-LUNAR-143 (the card content comes from the DayInfo already computed in TASK-LUNAR-007, not recomputed; the card needs no network)
...
risk_if_skipped: "The purple card-sharing feature is the visual highlight for the persona 'Chi Linh' (an actor who likes to post on Instagram). Without TASK-014, the app has no viral content to spread organically. TASK-009 already built the purple tokens but there is no export surface to bring them outside the app."
---
...
10. MUST provide `ShareCardSheet` - a bottom-sheet that lets the user choose a template and a date (defaulting to the day being viewed in the TASK-LUNAR-007 month calendar).
...
Upstream: TASK-LUNAR-007 provides `DayInfo` (lunarDate, canChiDay, ...) as the source of `CardData`; TASK-LUNAR-009 provides the purple color tokens - the hex values in `CARD_THEMES` MUST come from `packages/ui/src/theme/tokens.ts` and must not be separately hardcoded, to ensure consistency.

Downstream: no task is directly blocked by TASK-014.

Cross-cutting: `apca-w3` is a dev dependency of `apps/web/`; check that it is already in `packages/ui/` (TASK-LUNAR-009 uses it for the theme gate) to avoid duplication.
...
Still deferred: (a) support for an additional 9:16 format (1080x1920) for Instagram Story - not needed for slice 5, can be added after real feedback; (b) adding animation (a GIF/video card) - noted in the PRD Caveats but no task yet; (c) user-designed templates - out of scope for TASK-014.
...
- The color tokens in `CARD_THEMES` MUST be imported from `packages/ui/src/theme/tokens.ts` so that when TASK-LUNAR-009 updates the palette, the card updates automatically without editing two places.
