...
related_frs: [TASK-LUNAR-001, TASK-LUNAR-005, TASK-LUNAR-006]
depends_on: [TASK-LUNAR-001]
blocks: [TASK-LUNAR-005, TASK-LUNAR-006, TASK-LUNAR-016, TASK-LUNAR-017, TASK-LUNAR-018]
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-B02, TASK-B06)"
...
sub_tasks:
  ...
  - "1.5h: recurrence.test.ts - fixture for the 1985 leap month 2 death anniversary, a day-30 short-month death anniversary, MONTHLY 12 times, round-trip vs TASK-001"
  - "1.0h: index.ts barrel export + doc comment for the fallback policy and timezone lock"
risk_if_skipped: "Without this model there is nothing to remind - it is the data source that TASK-LUNAR-005 (the rolling-64 scheduler) and TASK-LUNAR-006 (reminder management) read from. If the recurrence is wrong, a grandparent's death anniversary is reminded on the wrong date every year, breaking the product's core duty. If Asia/Ho_Chi_Minh is not locked, a user traveling abroad sees the wrong lunar date. TASK-LUNAR-016/017/018 (Zalo, ZNS, family sync) all reuse exactly this Reminder type, so a schema error here spreads to the commercial downstream."
---
...
1. MUST store each repeating `Reminder` by `lunarDay` + `lunarMonth` + `isLeapMonth`, and MUST NOT store the solar date as the source value; the solar date is always derived via `convertLunar2Solar` (DEC-LUNAR-040, TASK-B02).
2. MUST support the `recurrence` ENUM `MONTHLY | ANNUAL | ONCE`: `ANNUAL` sets `lunarYear = null` (repeats yearly), `ONCE` keeps the entered `lunarYear` (fixed once), `MONTHLY` ignores `lunarMonth` and repeats `lunarDay` every lunar month (DEC-LUNAR-045, TASK-B02).
...
7. MUST handle a death anniversary falling on a day that does not exist in the target lunar month (for example day 30 when the target month has only 29 days): the engine MUST clamp back to the last day of that lunar month and mark `dayClamped = true` (TASK-B02 edge case).
8. MUST lock every calculation to `timeZone = 7.0` (meridian 105E, Asia/Ho_Chi_Minh) via `tz.ts`, and MUST NOT use the device's local-time `new Date()` to derive the lunar date or determine "today" (DEC-LUNAR-043, TASK-B06).
9. MUST assign each occurrence a `notifyTime` (default "07:00") and an array `leadTimes: number[]` in days (for example `[0, 1]` = on the day + 1 day before); each (occurrence x leadTime) pair generates its own reminder point (TASK-B04, preparing for TASK-LUNAR-005).
10. MUST generate each reminder point as an `Occurrence` carrying `reminderId`, `gregorianDate` (ISO date), `lunarLabel` (display string), `leadDays`, and `fireAtLocal` (datetime in Vietnam time) for TASK-LUNAR-005 to sort (PRD section 11).
...
14. MUST keep this module zero-dependency and purely computational (no I/O, no storage); reading/writing storage belongs to the app layer (TASK-LUNAR-010), sending notifications belongs to TASK-LUNAR-005 (NFR-Offline).
...
16. SHOULD expose `engineVersion` (the semver of amlich-core) so OccurrenceCache and TASK-LUNAR-018 (cloud sync) know when to recompute on the client.
...
**Why zero-dependency, purely computational (§1 #14)?** Same reason as TASK-LUNAR-001: three clients (web, iOS, Zalo) all import this module. Keeping it I/O-free means it runs identically everywhere, is testable with static fixtures, and does not pull in each platform's different storage API.
...
export interface User {
  ...
  theme: "purple";
  phone?: string;                  // cho ZNS (TASK-LUNAR-017)
...
export interface Reminder {
  ...
  readonly linkedContentId?: string;       // TASK-LUNAR-008 FestivalContent
  readonly sharedWith?: readonly string[]; // TASK-LUNAR-018 family sharing; optional theo CONTRACT
  readonly enabled: boolean;
  readonly notificationStyle?: NotificationStyle;  // TASK-F05 (TASK-LUNAR-006 fold); optional
}

/** TASK-F05 - tong giong + emoji cho body thong bao. TASK-LUNAR-006 set; TASK-LUNAR-005 doc; TASK-LUNAR-015 (P2) co the thay render. */
export interface NotificationStyle {
  tone: "warm" | "neutral" | "formal";
  emoji: string;
  imageId?: string;                // P1 chi emoji + anh dung san; upload rieng cho TASK-LUNAR-019 (PDPL)
}
...
/** Gop occurrence cua nhieu Reminder, sort theo fireAtLocal tang dan. TASK-LUNAR-005 cat 64 dau. */
...
8. **Matches leap year 1985** - a death anniversary entered in the leap month 2 of 1985 generates a solar-date occurrence falling in the range 21/03..19/04/1985 (cross-check TASK-LUNAR-001 fixture).
...
export function nextOccurrences(r: Reminder, opt: RecurrenceOptions): readonly Occurrence[] {
  ...
  while (lunarOccCount < opt.count && scanned < opt.count + MAX_SKIP_SCAN) {
    ...
    // 3) goi engine TASK-LUNAR-001 - tinh moi nam, khong noi suy (DEC-LUNAR-041)
...
`occurrenceBudget`/`opt.count` counts by LUNAR occurrence; each lunar occurrence generates `leadTimes.length` `Occurrence`. The loop increases `lunarOccCount` only when it actually generates an occurrence (not when it SKIPs), and has a `MAX_SKIP_SCAN` guard so a rare leap-month death anniversary with `leapFallback = SKIP` does not scan forever - the same spirit as the `i < 14` of `getLeapMonthOffset` in TASK-LUNAR-001.
...
- Upstream: TASK-LUNAR-001 (`convertLunar2Solar`, `convertSolar2Lunar`, `getLunarMonth11`, `getLeapMonthOffset`). This module does no astronomy of its own; it only calls the core engine and wraps the results in a data model.
- Downstream: TASK-LUNAR-005 (the rolling-64 scheduler reads the `Occurrence[]` after `mergeAndSort` and cuts the nearest 64), TASK-LUNAR-006 (reminder management CRUD on the `Reminder` type, showing the upcoming list), TASK-LUNAR-016 (Zalo Mini App reuses Reminder + computes dates on the fly), TASK-LUNAR-017 (ZNS scans Reminders with the `ZNS` channel), TASK-LUNAR-018 (cloud sync syncs Reminder + `sharedWith`).
- Cross-cutting: `engineVersion` relates to TASK-LUNAR-001 (the core semver) and TASK-LUNAR-018 (invalidate the client cache when the core changes). `linkedContentId` points to TASK-LUNAR-008 FestivalContent.
...
Deferred (close to the Caveats / later tasks):
...
- A region-specific fallback rule (some households observe the leap month rather than the regular month) - left for TASK-LUNAR-008 to connect the customs content; this version only offers the 3 policies REGULAR/SKIP/ASK.
- Accuracy in very distant years (Caveats: a Soc point near midnight) - the responsibility of TASK-LUNAR-003 (the golden harness); this module only calls the engine and trusts its result.
...
- `fireAtLocal` always carries a hard `+07:00` offset and every date comparison uses ISO strings. Avoid `new Date(str)` and then reading `getDate()` because it reads by the runtime TZ - exactly the reason TASK-B06 requires locking Asia/Ho_Chi_Minh.
- `engineVersion` should come from the amlich-core `package.json` and be passed into `RecurrenceOptions`, so OccurrenceCache (DEC-LUNAR-044) and TASK-LUNAR-018 share a single source of truth for the core version.
...
- Keep `Occurrence` the only structure that TASK-LUNAR-005 reads. Do not let the scheduler derive dates itself - all the lunar logic lives here, the scheduler only merges, sorts, and cuts 64.

---

*End of TASK-LUNAR-004.*
