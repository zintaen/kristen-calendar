...
related_frs: [TASK-LUNAR-005, TASK-LUNAR-009]
depends_on: [TASK-LUNAR-001, TASK-LUNAR-009]
blocks: [TASK-LUNAR-005, TASK-LUNAR-006, TASK-LUNAR-007, TASK-LUNAR-012, TASK-LUNAR-015]
source_pages:
...
sub_tasks:
  ...
  - "1.0h: app/calendar/page.tsx - route wrapping CalendarGrid (TASK-007)"
  - "1.0h: app/reminders/page.tsx and settings/page.tsx - placeholder routes for TASK-006"
...
risk_if_skipped: "Without the app shell, there is nothing to deploy for the wife to try (an MVP criterion). TASK-005 (rolling-64 notifications) and TASK-006 (reminder management) both depend on the app shell for the storage layer and the Capacitor plugin glue. TASK-007 (grid) needs routing to mount CalendarGrid on the /calendar route."
---
...
// TASK-LUNAR-004 so huu kieu Reminder (bao gom truong optional notificationStyle).
...
2. The `/festival/[id]` route with `id = "vu-lan"` renders the Vu Lan detail page taken from `getFestivalById("vu-lan")` of TASK-LUNAR-008 without server-side data fetching.
...
8. `public/manifest.json` is valid JSON, with all 4 fields `name`, `display: "standalone"`, `theme_color`, `background_color`; `background_color` is the warm cream value from TASK-LUNAR-009.
...
// testReminder phai co day du truong bat buoc cua Reminder (CONTRACT.md / TASK-LUNAR-004):
...
// KHONG phai object; can-chi/zodiac den tu cac ham TASK-LUNAR-002 (canChiDay/canChiYear nhan JDN,
...
Upstream: `TASK-LUNAR-001` provides `convertSolar2Lunar` used on the home route `/` to compute today's lunar date; `TASK-LUNAR-009` provides `PURPLE_TOKENS`, `SEMANTIC`, and components to apply the theme globally in `layout.tsx`.

Downstream: `TASK-LUNAR-005` (rolling-64 notifications) uses the `NotificationService` interface from `notificationGlue.ts` and `Reminder[]` from `storage.ts`; `TASK-LUNAR-006` (reminder management) uses the storage CRUD and the `/reminders` route; `TASK-LUNAR-007` (calendar grid) mounts on the `/calendar` route; `TASK-LUNAR-012` (good-day picker) mounts on a new route in Phase 2; `TASK-LUNAR-015` (AI Genie) needs a Next.js API route - it will change `output: "export"` to hybrid mode or split out to Vercel Functions when Phase 2 begins.

Cross-cutting: All Phase 1 tasks depend on `storage.ts` to read/write Reminders. `notificationGlue.ts` is the contract that TASK-LUNAR-005 implements concretely on Capacitor.
...
| createNotificationService() returns the wrong type | Unit test | TASK-005 calls a Capacitor API on the web, crash | Fix the isCapacitor() check |
...
- The home route `/` assembles the lunar date like TASK-LUNAR-007: `convertSolar2Lunar` returns a TUPLE `[d,m,y,leap]` (no can-chi/zodiac). Can-chi/zodiac MUST be called separately from TASK-LUNAR-002 with a JDN: `const jdn = jdFromDate(d,m,y); canChiDay(jdn).label; canChiYear(lYear).label; zodiacOf(lYear)`. The §5 mock test has been fixed to match this form - avoiding a false-green when the mock object-shape differs from the real contract.
