...
convertLunar2Solar with a wrong-year leap flag needs a sentinel. Resolved: §1 #2, AC #16, returns [0,0,0], §8 payload 3, §10 line 8 (TASK-LUNAR-004 applies the fallback).
...
All 7 substantive concerns are handled: the three epochs and two synodic constants are separated with JSDoc, the timezone is locked to 105E, off-by-one is blocked by the round-trip sweep, the leap sentinel connects to TASK-LUNAR-004, offline and zero-dependency have guard tests, and the constants are frozen. The functions are ported verbatim from the canonical Ho Ngoc Duc source with the exact PRD 6.2 constants. **Score = 10/10.** Ready to transition draft -> ready_to_implement.
