...
related_frs: []
depends_on: [TASK-LUNAR-020, TASK-LUNAR-005]
blocks: []
source_pages:
  - BACKLOG.md
source_decisions:
...
The app is currently entirely free. Although Affiliate (TASK-LUNAR-022) generates cash flow, IAP (direct sales) is still the most stable and scalable monetization model for utility apps (Calendar/Widget). Users have a high demand for personalization (changing the app color to match their feng shui element) but have no way to pay for that feature.
...
- **Upstream:** TASK-LUNAR-005 (Widget engine - to support premium widget rendering).
- **Upstream:** TASK-LUNAR-020 (Account management - to link purchases to Supabase UUIDs).
...
- `Resolved` - Should we use RevenueCat's anonymous IDs or require login to buy? Resolution: Require login (TASK-020) so purchases can sync cross-platform (iOS to Android).
