...
related_frs: [TASK-LUNAR-002, TASK-LUNAR-011]
depends_on: [TASK-LUNAR-001, TASK-LUNAR-002, TASK-LUNAR-010]
blocks: [TASK-LUNAR-014]
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-A05)"
...
source_decisions:
  ...
  - DEC-LUNAR-072 (tapping a day cell opens a detail modal/panel showing the full DayInfo from cache; this panel is a placeholder for TASK-LUNAR-011 Hoang dao/Truc/28 mansions when available)
...
risk_if_skipped: "Without the calendar view, the user cannot see an overview of the lunar dates in the month - the main screen of the MVP app. TASK-LUNAR-014 (shareable cards) depends directly on this task's DayCell component to render the card design."
---
...
1. **MUST** display a 7-column grid (Sunday to Saturday), each row a solar week, each cell corresponding to one solar day of the month (TASK-A05).
2. **MUST** display in each cell: the solar date (large, prominent number), the corresponding lunar date (small number, bottom-left corner), and the day's can-chi (small text below the solar date) (TASK-A05, DEC-LUNAR-070).
3. **MUST** mark the day's tiet khi with a small label or icon on the cell when that day is the start day of one of the 24 tiet khi (TASK-A05, TASK-A04 from TASK-LUNAR-002).
...
14. The `DayDetailPanel` **MUST** display at least: the full solar date, the full lunar date (leap month if any), the day/month/year can-chi, the tiet khi if any, and the list of reminders on that day; the Hoang dao/Truc/28 mansions fields **SHOULD** be placeholders for TASK-LUNAR-011 when ready (DEC-LUNAR-072).
...
**Why limit to 3 dot levels (DEC-LUNAR-073)?** A calendar UI gets cluttered easily if every kind of reminder has a different color. Three levels (blank / ordinary reminder / major festival) are enough for the user to scan quickly without reading each cell. The purple design of the TASK-LUNAR-009 sub-brand supplies the primary and accent colors.

**Why is `DayDetailPanel` a placeholder for TASK-LUNAR-011 (DEC-LUNAR-072)?** TASK-LUNAR-011 (Hoang dao/Truc/28 mansions) depends on TASK-LUNAR-002 and belongs to Phase 2. If TASK-007 hard-wired its layout to TASK-011, the absence of TASK-011 would block TASK-007. A placeholder design lets TASK-007 ship in Phase 1 slice 3, and TASK-011 only needs to fill the reserved slot when ready.
...
**Why test fixtures for Jan/2025?** 29/01/2025 is the first day of Tet At Ty - if the grid shows the wrong lunar date 1/1 for this day, the whole month is wrong. The fixture already exists in TASK-LUNAR-003 (golden validation), reused for the component test.
...
```typescript
// apps/web/lib/calendarData.ts
// TASK-LUNAR-001 exports `convertSolar2Lunar` (returns a TUPLE [day, month, year, leap])
// and `jdFromDate`. TASK-LUNAR-002 exports `canChiDay(jdn)`, `canChiMonth(lunarMonth, lunarYear)`,
...
export interface DayCellData {
  ...
  isFestival: boolean;
  isToday: boolean;
  // Placeholder fields for TASK-LUNAR-011:
...
1. The Jan/2025 month grid shows 29/01/2025 with the lunar date label "1/1" (first day of Tet At Ty), and the can-chi correct per the TASK-LUNAR-003 fixtures.
...
6. Today's cell (solarDay == today) has a distinct highlight style (a deep purple ring or equivalent from the TASK-LUNAR-009 design token).
...
14. The tiet khi "Lap Xuan" shows correctly on the cell for 3/2/2025 (or the exact day from TASK-LUNAR-002).
15. The grid works correctly in both modes: light (default) and dark if the TASK-LUNAR-009 design system supports it.
...
//     co trong tuple; phai goi rieng cac ham TASK-LUNAR-002 voi JDN (canChiDay/tietKhiAt nhan JDN).
...
Upstream: `TASK-LUNAR-001` provides `convertSolar2Lunar` (the core engine, returns a tuple) and `jdFromDate`; `TASK-LUNAR-002` provides `canChiDay/canChiMonth/canChiYear`, `zodiacOf`, and `tietKhiAt` (24 tiet khi - NOT `getTietKhi`); `TASK-LUNAR-010` provides the app shell, routing, and the storage layer that `CalendarGrid` uses to read the list of active `Reminder`s.

Downstream: `TASK-LUNAR-014` (shareable cards) depends on the `DayCell` component and the `DayCellData` type from this task to render the card design.

Cross-cutting: `TASK-LUNAR-009` (purple design system) provides the color tokens and typography for `DayCell` and `DayDetailPanel`; `TASK-LUNAR-011` (Hoang dao/Truc/28 mansions) will fill the placeholder fields `hoangDao`, `truc`, `sao28` in `DayCellData` in Phase 2 when ready.
...
- Placeholder for TASK-LUNAR-011: the `hoangDao`, `truc`, `sao28` fields in `DayCellData` stay `null` until Phase 2.
...
|---|---|---|---|
| `convertSolar2Lunar` returns the wrong leap month | Unit test fixtures for Mar/1985 | Grid shows the wrong lunar date | Fix the bug in TASK-LUNAR-001 before deploy |
...
| TASK-LUNAR-009 color tokens not ready | Import build error | Grid has no purple style | Use a fallback CSS variable default in DayCell |
...
- The tiet khi from TASK-LUNAR-002 is `tietKhiAt(jdn, tz)` (there is NO `getTietKhi`), and it returns the tiet khi for EVERY day (always has a `name`), not just the start day. The grid should show the tiet label only on the START DAY of the tiet; §6 uses `isTietKhiStart(jdn)` comparing `tietKhiAt(jdn).index` with `tietKhiAt(jdn-1).index` (different = start day) before assigning `tietKhi`, leaving the rest `null`. AC #14 (Lap Xuan on 3/2/2025) tests exactly this behavior.
