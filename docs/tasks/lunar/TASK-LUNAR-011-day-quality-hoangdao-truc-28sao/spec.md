...
related_frs: [TASK-LUNAR-002, TASK-LUNAR-012]
depends_on: [TASK-LUNAR-002]
blocks: [TASK-LUNAR-012, TASK-LUNAR-013]
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-E02, TASK-E03)"
...
sub_tasks:
  ...
  - "1.5h: function truc(jdn, tietKhiIndex) per DEC-LUNAR-113; determine the current solar term from TASK-LUNAR-002"
...
risk_if_skipped: "TASK-LUNAR-012 (good-day picker) cannot be built at all because it is only UI over DayQuality. TASK-LUNAR-013 (widget) will lack the auspicious-hours data - one of the two most important information fields of the widget. The users are actors and business owners - the main personas who need to view good days - and they would not have this feature."
---
...
5. MUST compute `gioHoangDao` for the 12 gio of the day by looking up `GIO_HOANG_DAO_TABLE[diaChiNgay]`, returning an array of 12 `GioInfo { canh: string, tuGio: string, denGio: string, isHoang: boolean }` (DEC-LUNAR-115, PRD §8, TASK-E03).
6. MUST export the main function `getDayQuality(solarDate: Date): DayQuality`, which takes `canChiNgay` and the JDN from amlich-core (TASK-LUNAR-002), computes all fields, and sets `disclaimer = "Tham khao phong thuy dan gian"` at the root level of the result (DEC-LUNAR-110).
7. MUST obtain the day's can-chi and `diaChiNgay` by calling `canChiDay(jdn)` of TASK-LUNAR-002 (with `jdn` from `jdFromDate` of TASK-LUNAR-001), using `canChiDay(jdn).chiIndex = (jdn + 1) mod 12` as `diaChiSoNgay` and `canChiDay(jdn).label` as `canChiNgay`; MUST NOT derive the earthly branch by `(jdn + 9) mod 60` then `mod 12` (which yields `(jdn + 9) mod 12`, off by 8 relative to core) and MUST NOT import a third-party library to recompute it (DEC-LUNAR-112). MUST compute `diaChiSoDauTiet` (the earthly branch of the first day of the current solar term) by calling `tietKhiStartDiaChi(jdn, tz?)` of TASK-LUNAR-002 (CONTRACT.md P2/P3 surface); the full Truc formula is `(canChiDay(jdn).chiIndex - tietKhiStartDiaChi(jdn) + 12) % 12` for the index into `TRUC_NAMES` (DEC-LUNAR-113). Because `THAN_TRUC_NHAT_TABLE`, Truc, and the auspicious hours are all keyed on the day's earthly branch, a wrong earthly branch corrupts the entire day-quality result and diverges from the can-chi displayed in the calendar (TASK-LUNAR-007 via TASK-LUNAR-002).
8. MUST compute `thangAmIndex` (0..11) from the lunar month returned by TASK-LUNAR-001, to index into `THAN_TRUC_NHAT_TABLE` - the index is (thangAm - 1) mod 12 (DEC-LUNAR-111).
...
10. MUST NOT call the network at any step; MUST NOT cache results to IndexedDB or localStorage (those steps are the responsibility of TASK-LUNAR-010); this module is pure compute only (DEC-LUNAR-110).
11. MUST export all enums and types from `packages/amlich-core/src/index.ts` so that other tasks (TASK-LUNAR-007, TASK-LUNAR-012, TASK-LUNAR-013) can import them directly.
...
14. MAY export the function `getMonthDayQualities(year: number, month: number): readonly DayQuality[]` to quickly compute the whole month - used by TASK-LUNAR-012 to list Hoang dao days within a range (DEC-LUNAR-110).
...
**Why export `getMonthDayQualities` (DEC-LUNAR-110)?** TASK-LUNAR-012 needs to list Hoang dao days within a range (for example July 2026). If TASK-012 called `getDayQuality` day by day in a loop, although more efficient than calling the network it is still ~30 computations. The `getMonthDayQualities` function gathers this in one place, allows future optimization if needed (e.g. batch JDN lookup), and makes the TASK-012 contract simpler.
...
// Main public API
// getDayQuality PHẢI lấy can-chi/địa chi và địa chi đầu tiết từ TASK-LUNAR-002:
...
16. The `diaChiNgay` of `getDayQuality` MUST equal the earthly branch of `canChiDay(jdn)` (TASK-LUNAR-002) for every day across a multi-day scan (>= 60 consecutive days): for each day, the `DiaChi` index of `q.diaChiNgay` === `canChiDay(jdFromDate(d,m,y)).chiIndex` and `q.canChiNgay` === `canChiDay(...).label`. (Catches the earthly-branch off-by-8 caused by `(jdn+9)%60` - DEC-LUNAR-112.)
17. For the Tet 2025 fixture (2025-01-29), the `diaChiNgay` of `getDayQuality` matches the earthly branch of `canChiDay` for that day (MUST NOT be off); this is the same value the TASK-LUNAR-007 calendar cell displays.
...
// AC #16/#17 - dia chi PHAI nhat quan voi TASK-LUNAR-002 canChiDay (DEC-LUNAR-112).
...
The API contract in §3 is the main skeleton. The key point to state clearly: the `getDayQuality` function calls `jdFromDate` from TASK-LUNAR-001, then calls `canChiDay(jdn)` of TASK-LUNAR-002 and takes `const cc = canChiDay(jdn); const diaChiIndex = cc.chiIndex; // = (jdn + 1) % 12; canChiNgay = cc.label`. NEVER derive the earthly branch by `(jdn + 9) % 60` then `% 12` (which gives `(jdn + 9) % 12`, off by 8 relative to core). Index into `THAN_TRUC_NHAT_TABLE[diaChiIndex][thangAmIndex]`. Truc is computed via `tietKhiStartDiaChiIndex` taken from the `tietKhiStartDiaChi(jdn, tz?)` function of TASK-LUNAR-002 (CONTRACT.md: `export function tietKhiStartDiaChi(jdn: number, tz?: number): number`), then `(diaChiIndex - tietKhiStartDiaChiIndex + 12) % 12` gives the index into `TRUC_NAMES` (DEC-LUNAR-113). The 28 sao are computed as `(jdn - BASE_JDN_GIAC + 2800) % 28`. Because all three tables (daily officer deity, Truc, auspicious hours) are keyed on `diaChiIndex`, taking the earthly branch consistently with core is the single most important invariant of this module.

---

## §7 - Dependencies

Upstream: TASK-LUNAR-002 is a required dependency. `getDayQuality` needs `jdFromDate` (TASK-LUNAR-001, re-exported via TASK-LUNAR-002), `canChiDay` (TASK-LUNAR-002), and `tietKhiStartDiaChi(jdn, tz?)` (TASK-LUNAR-002, CONTRACT.md P2/P3 surface) to determine `diaChiSoDauTiet` for computing Truc by the formula `(canChiDay(jdn).chiIndex - tietKhiStartDiaChi(jdn) + 12) % 12` (DEC-LUNAR-113). All of these functions are available once TASK-LUNAR-002 is complete.

Downstream: TASK-LUNAR-012 (good-day picker) depends entirely on `getDayQuality` and `getMonthDayQualities`. TASK-LUNAR-013 (widget) uses `getDayQuality` to display `canChiNgay`, `label` (Hoang dao/Hac dao), and `gioHoangDao`. TASK-LUNAR-007 (month grid) uses `isHoangDao` and `truc.name` to display in the day cell.

Cross-cutting: the `disclaimer` field ensures the "folk feng shui reference" label is passed down to every UI layer from the root data, without TASK-007/012/013 needing to add the label themselves.
...
| JDN from TASK-LUNAR-001 off by 1 | Test AC #1 fixture (tet 2025) fails | Sao/Truc/DiaChi off by 1 | Fix TASK-001 first, do not fix here |
...
| A new enum not re-exported from index.ts | TASK-012/013 build fails | Import error | Add it to the index.ts export |
...
| TASK-LUNAR-002 not ready (wrong build order) | TypeScript compile error | Build fails | Build TASK-002 first |
