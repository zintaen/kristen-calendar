...
related_frs: [TASK-LUNAR-004, TASK-LUNAR-006]
depends_on: [TASK-LUNAR-004, TASK-LUNAR-010]
blocks: [TASK-LUNAR-006]
source_pages:
  - "docs/PRD + SRS — Ứng Dụng Nhắc Âm Lịch Việt Nam (\"Genie Âm Lịch\" của CyberSkill).md#4 (TASK-B05)"
...
source_decisions:
  ...
  - DEC-LUNAR-056 (the scheduler is a pure planner + thin native adapter: the planner computes a testable plan, the adapter only calls Capacitor schedule/cancel; all occurrences come from TASK-LUNAR-004, the scheduler does not derive lunar dates itself)
...
disallowed_tools:
  ...
  - deriving lunar dates in the scheduler instead of reading Occurrence from TASK-LUNAR-004 (violates DEC-LUNAR-056)
effort_hours: 10
sub_tasks:
...
risk_if_skipped: "Without the scheduler, even if TASK-LUNAR-004 generates the correct occurrences, the device never rings - the app cannot remind anything, which is the PRD's G1 goal. If more than 64 pending are scheduled on iOS, the system silently drops them and the user loses exactly the farthest reminders - hard to detect, loss of trust. If it depends on BGAppRefreshTask as a guaranteed channel, users who rarely open the app miss slots. TASK-LUNAR-006 (reminder management) calls reschedule whenever the user adds/edits/toggles a reminder, so a stable API is required here."
---

## §1 - Description (BCP-14 normative)

The scheduler turns the list of `Occurrence` (from TASK-LUNAR-004) into local notifications, honoring iOS's 64-pending ceiling with a rolling reschedule strategy. This is the contract:

1. MUST, on each reschedule, call `removeAllPendingNotificationRequests()` (cancelAll) first, then `add()` at most the nearest 64 reminder points from all `enabled` Reminders; MUST NOT let the total pending exceed 64 on iOS (DEC-LUNAR-050, TASK-B05).
2. MUST read the occurrences from TASK-LUNAR-004 (`nextOccurrences` then `mergeAndSort`) and MUST NOT derive lunar dates in the scheduler; the scheduler only merges, sorts by `fireAtLocal`, and cuts the nearest 64 (DEC-LUNAR-056).
...
12. MUST set each notification's `fireAtLocal` to the correct Vietnam time from the Occurrence (`+07:00`), and MUST NOT convert timezones in the scheduler; the timezone is already locked in TASK-LUNAR-004 (DEC-LUNAR-056, TASK-B06).
...
14. MUST expose `getPlanDiagnostics()` returning `{ scheduled, slotsDropped, remindersCovered, horizonMonths }` for the debug screen and for TASK-LUNAR-006 to show "you have too many reminders, some will be re-scheduled as they get close".
15. SHOULD attach the notifications of pendingUserChoice/fellBack occurrences (from TASK-LUNAR-004) with a flag in `userInfo` so the detail screen can prompt the user to confirm the leap-month fallback.
...
describe("rolling-64 planner", () => {
  ...
  it("fireAtLocal preserves +07:00 (TASK-B06, AC #11)", () => {
...
export function planSchedule(reminders, nowUtcMs, opts): SchedulePlan {
  ...
  // 1) sinh occurrence trong horizon cho moi reminder enabled (TASK-LUNAR-004), bo qua khu
...
- Upstream: TASK-LUNAR-004 (`nextOccurrences`, `mergeAndSort`, the `Reminder`/`Occurrence` types, `engineVersion`) is the source of every reminder point; TASK-LUNAR-010 (the app shell provides the Capacitor host, the storage layer that reads the Reminder list, and calls `reschedule` on app open/onResume).
- Downstream: TASK-LUNAR-006 (reminder management calls `reschedule` whenever the user adds/edits/deletes/toggles a reminder, and uses `getPlanDiagnostics` to warn when there are too many reminders).
- Cross-cutting: the deep-link route `/reminder/:id` matches the TASK-LUNAR-006 detail screen; the `fellBack`/`pendingUserChoice` flags come from TASK-LUNAR-004 for the UI to prompt the leap-month fallback confirmation.
...
- The reliability of BGAppRefreshTask for users who rarely open the app (Caveats: the risk of missing slots) - must be tested in practice on a device, and could be supplemented with a periodic reminder via ZNS (TASK-LUNAR-017) as a safety net in the commercial version.
...
| Wrong timezone when scheduling | keep +07:00 from the Occurrence | correct VN time | none (TASK-B06; AC #11) |
