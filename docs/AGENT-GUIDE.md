...
The entire spec lives in `docs/tasks/`: 20 tasks `TASK-LUNAR-001..020` (each an 11-section engineering-spec with a sibling `*.audit.md`), a `lunar/README.md` (index + build order + PRD traceability), `lunar/manifest.json` (machine-readable), `lunar/INDEPENDENT-AUDIT-2026-06-27.md` (independent audit), and `BACKLOG.md`. The original PRD/SRS is at `docs/PRD + SRS — ...md`. When implementing an task, read that spec's section 3 (API contract) and section 5 (Verification) first.

## Monorepo layout

```
packages/amlich-core/   # TASK-001/002/003/004 - TS, zero-dependency, offline, unit-tested. LOI CUA SAN PHAM.
packages/content/       # TASK-008 - FestivalContent tinh (13 dip)
packages/ui/            # TASK-009 - purple theme pack, APCA gate, Be Vietnam Pro
apps/web/               # TASK-010 shell, 006/007/012/014/015 UI - Next.js/React PWA + Capacitor (ios/App: 005 notif, 013 widget Swift)
zalo/                   # TASK-016 - Zalo Mini App (React + zmp-ui + zmp-sdk)
services/genie-api/     # TASK-015 Claude proxy, 017 ZNS, 018 sync, 019 PDPL, 020 billing - serverless TS
```
...
3. An task moves to `done` only after it passes the gate (tests green + typecheck + review). Do not flip status on your own. The operator (Stephen) runs the final gate and does the git commit on the real machine.
...
- Can-chi of the day: `can = (jdn + 9) % 10`, `chi = (jdn + 1) % 12` (TASK-002 is the owner). Day-quality (TASK-011/013) MUST take the dia chi from `canChiDay(jdn).chiIndex`, MUST NOT derive it from `(jdn+9)%60 % 12` (off by +8).
...
- Build order + PRD traceability: `docs/tasks/lunar/README.md`
- Backlog + founder decisions (locked): `docs/tasks/BACKLOG.md`
- Independent audit (defects fixed + open items): `docs/tasks/lunar/INDEPENDENT-AUDIT-2026-06-27.md`
