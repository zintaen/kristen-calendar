...
Để phát triển và kiểm thử cục bộ, môi trường làm việc sử dụng `pnpm workspaces` với các lệnh chính:
...
2. **Kiểm tra tính đúng đắn** (Typecheck và Unit Test):
   ```bash
   pnpm --filter @cyberskill/amlich-core typecheck
   pnpm --filter @cyberskill/amlich-core test
   ```
   *Lưu ý: Core P0 phải luôn pass 100% test case (đặc biệt dải năm 1900-2199).*

3. **Database & Backend Cục bộ (Supabase)**:
   ```bash
   ...
   ```
   *Cấu hình `.env` cho web và api sử dụng URL và anon key từ output của `supabase status`.*

4. **Khởi chạy ứng dụng (chế độ Dev)**:
   Mở các terminal riêng biệt để chạy các ứng dụng:
   ```bash
   # Chạy Next.js Web App (http://localhost:3000)
   pnpm --filter web dev
   
   # Chạy Zalo Mini App
   pnpm --filter zalo dev
   
   # Chạy Genie API (http://localhost:3001)
   pnpm --filter genie-api dev
   ```

## Hướng dẫn triển khai (Deployment Guidelines) với Docker

Dự án hỗ trợ container hóa thông qua Docker Compose để đồng nhất môi trường triển khai cho API và Web (Next.js Static Export phục vụ qua Nginx).

**Bước 1: Chuẩn bị biến môi trường**
Tạo file `.env.docker` tại thư mục gốc dự án để cấp cho các containers:
```bash
# Ví dụ nội dung .env.docker
NEXT_PUBLIC_SUPABASE_URL=...
NEXT_PUBLIC_SUPABASE_ANON_KEY=...
ANTHROPIC_API_KEY=...
```

**Bước 2: Xây dựng (Build)**
Hệ thống sử dụng Alpine Linux và sẽ tự động cài các package hệ thống cần thiết (như python3, cairo, pango cho node-canvas).
```bash
...
- `genie-api`: Chạy tại cổng **3000** (nội bộ hoặc expose tùy config proxy ngoài).
- `web`: Frontend tĩnh (Nginx) tại cổng **8080**.
Truy cập ứng dụng web thông qua: `http://localhost:8080`
