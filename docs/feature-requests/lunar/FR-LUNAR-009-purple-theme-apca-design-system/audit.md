...
After handling the 7 issues above, FR-LUNAR-009 has 16 BCP-14 clauses, 15 ACs, 9 unit tests including a "fail gate" test confirming assertApca works correctly, 10 failure rows, 6 implementation notes. All of DEC-LUNAR-090..094 are created and fully referenced. Score after self-audit = 10/10.

## §4 - Independent adversarial pass (2026-06-27)

Pre-fix score: **9/10** - the cleanest FR in the set. The independent reviewer RE-COMPUTED APCA with the reference apca-w3 formula so as not to rubber-stamp the gate; every assertion is correct:
...
- **No amlich-core imports**: FR-009 imports nothing from amlich-core (it is a design-system package, not dependent on amlich). PASS.
