---
id: FR-LUNAR-025
title: "B2B API (Calendar as a Service) - API cho doanh nghiệp"
module: LUNAR
priority: COULD
...
source_pages:
  - BACKLOG.md
source_decisions:
  - DEC-025 (Mở rộng sang mảng B2B, cung cấp API tính toán lịch Âm cho các đối tác)
language: typescript
...
effort_hours: 15
sub_tasks:
  - "3h: Thiết kế schema lưu trữ API Keys cho đối tác B2B"
  - "3h: Viết middleware xác thực API Key và đếm quota usage (Rate Limit)"
  - "4h: Expose các endpoints: /v1/lunar/convert, /v1/lunar/events"
  - "2h: Viết API Documentation (Swagger/OpenAPI)"
  - "3h: Thiết lập monitoring dashboard cho B2B API usage"
risk_if_skipped: "Bỏ lỡ mảng B2B. Các nhà hàng chay, cửa hàng hoa có nhu cầu rất lớn về việc gửi tin nhắn tự động cho khách hàng vào ngày Rằm, nhưng họ không có API lịch âm chính xác để tự phát triển."
---
...
Xây dựng một public REST API (B2B API) cung cấp dịch vụ "Calendar as a Service". Các doanh nghiệp đối tác (nhà hàng chay, chuỗi bán lẻ, dịch vụ tâm linh) có thể đăng ký API Key và gọi vào hệ thống của Genie để: chuyển đổi ngày Dương/Âm, lấy danh sách các ngày lễ Âm lịch trong tháng, tính toán ngày tốt xấu (nếu có). API này sẽ được thu phí theo model SaaS (Pay-as-you-go hoặc Tiered Subscription).

## Problem

Nhiều doanh nghiệp F&B (đặc biệt là nhà hàng chay) muốn chạy chiến dịch marketing (SMS, Zalo ZNS) tự động nhắc khách hàng đặt bàn vào mùng 1 và ngày rằm hàng tháng. Tuy nhiên, việc tính toán chính xác ngày rằm Âm lịch theo chuẩn Việt Nam (múi giờ UTC+7, thuật toán Hồ Ngọc Đức) không hề đơn giản. Họ cần một API ổn định để gọi thay vì tự xây dựng lại core engine.

## Customer Quotes

<untrusted_content source="b2b-survey"> "Bên mình đang dùng CRM tích hợp gửi SMS, nhưng toàn phải nhập tay ngày rằm mỗi tháng. Nếu có API trả về mùng 1 và ngày rằm của tháng tới thì mình cho dev cắm vào CRM luôn." </untrusted_content>
