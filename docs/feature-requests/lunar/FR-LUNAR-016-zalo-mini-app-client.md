...
source_decisions:
  ...
  - DEC-LUNAR-162 (Mini App cannot send native-style push notifications itself; every Zalo reminder channel MUST use ZNS in FR-LUNAR-017)
  ...
  - DEC-LUNAR-165 (the purple theme from FR-LUNAR-009 is applied via CSS variables; zmp-ui components are wrapped or styled to fit the purple sub-brand)
...
sub_tasks:
  ...
  - "1.0h: styling - apply the purple theme tokens from FR-LUNAR-009 to zmp-ui components"
risk_if_skipped: "Without a Zalo Mini App, we cannot reach the roughly 80 million Zalo users through Vietnam's strongest distribution channel (Key Findings 3). FR-LUNAR-017 (ZNS) depends directly on this FR to obtain phone numbers and the list of Reminders that consented to ZNS; without a Zalo client, the entire Phase 3 commercialization is blocked. FR-LUNAR-019 (PDPL) also needs the consent screens built here."
---
...
8. MUST call `apis.getPhoneNumber()` only when the user enables the ZNS channel in Settings AND has confirmed ZNS consent; the phone number is stored only in the `User.phone` field to pass to the ZNS sender (FR-LUNAR-017), MUST NOT be used for any other purpose (DEC-LUNAR-163).
...
10. MUST display the `FestivalDetail` page for each lunar festival, loading content from `@cyberskill/genie-content` (`FestivalContent`); ensure the link from a Reminder to FestivalDetail through `linkedContentId` (FR-LUNAR-008).
11. MUST apply the purple theme from `@cyberskill/genie-ui` (tokens from FR-LUNAR-009) to the `zmp-ui` components; MUST NOT use the default `zmp-ui` colors for the main elements (DEC-LUNAR-165).
12. SHOULD get `getLocation` (location) to suggest the local time zone if the user is abroad; however, all lunar calendar computation MUST be locked to `timezone = "Asia/Ho_Chi_Minh"` (UTC+7, meridian 105E) per DEC-LUNAR-160 and FR-LUNAR-004.
13. MUST NOT send a push notification to the user from inside the Mini App; every reminder function must go through ZNS (FR-LUNAR-017) once a valid phone number and consent are available (DEC-LUNAR-162).
...
**Why not send push notifications directly?** This is a hard technical limitation of the Zalo Mini App platform - a native push API does not exist for Mini Apps. Every Zalo reminder must go through the ZNS of a registered OA. The FR-016 design recognizes this from the start and shifts reminder responsibility entirely to FR-LUNAR-017 (DEC-LUNAR-162).

**Why the purple theme from FR-LUNAR-009 instead of the zmp-ui default?** The purple sub-brand is the product identity and a consistent user experience across every platform (FR-LUNAR-009, PRD §13). zmp-ui can be styled via CSS variables without breaking Zalo's guidelines. The override applies only to color tokens, keeping the component structure intact so the Mini App still passes review (DEC-LUNAR-165).
...
1. The Mini App starts successfully in Zalo DevTools and on a real device; `zmp.init()` does not error; the Home screen displays today's lunar date correctly (cross-checked against the FR-LUNAR-003 fixture set).
...
The hardest point is that Zalo's `getPhoneNumber` returns a `token`, not the real phone number. The real number is only obtainable when the server calls the OA API with that token. This logic must live on the server side (FR-LUNAR-017); the client only holds the token and forwards it when initializing the ZNS subscription.

---

## §7 - Dependencies

Upstream: **FR-LUNAR-004** (the `Reminder` data model and the `recurrence` logic referenced in `ZaloReminder`; amlich-core is a product of FR-001 that FR-004 depends on). **FR-LUNAR-008** (`FestivalContent` from `packages/content` used for FestivalDetail and the colored dots on the CalendarGrid). **FR-LUNAR-009** (the purple theme tokens imported into the Zalo Mini App styling).

Downstream: **FR-LUNAR-017** (the ZNS sender needs `User.phone` (token) and the list of `Reminder`s with `channels: ["ZNS"]` built in this FR). **FR-LUNAR-019** (the PDPL consent layer needs the screens and the `consentFlags` built in this FR).
